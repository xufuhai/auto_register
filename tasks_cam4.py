  ...
  try:
    ...
    try:
        if ostype == 'PC':
            ...
            #await page.wait_for_selector('button[data-id="Create Free Account"]')
            await random_click_element(page, 'button[data-id="Create Free Account"]')
    ...
    await page.wait_for_selector('input#registration-username', timeout=150000)
    num_arrays = [1, 2, 3, 4, 5, 6, 7, 8, 9, 0]
    usernum = random.choice(num_arrays)
    await page.fill('input#registration-username', username + str(usernum))
    ...
    await page.wait_for_selector('input#registration-password', timeout=150000)
    await page.fill('input#registration-password', password)
    await random_pause()
    # 确保复选框元素可见
    ...
    await random_click_element(page, 'input#registration-consent-checkbox')
...
def register_user_task(url, email, password, proxy_ip, user_agent, country, city, username, ostype):
    ...
    if result == 1:
        insert_registration_task(url, email, password, proxy_ip, user_agent, country, city, 'cam4')
...
browser_id, ip, country, city, postal, ostype = createBrowser(False, '', 'cam4')
detailes = detailBrowser(browser_id)
...
url='https://t.ajrkm.link/328040/5409/0?bo=2779,2778,2777,2776,2775&po=6533&aff_sub5=SF_006OG000004lmDN'
#url=sys.argv[1]
print(url)
username=generate_username()
#url='https://sweetydating.online/im/click.php?c=17&key=8ql18onn77ez91a7128cz2ga'
email = get_random_email_with_status_zero('4')
